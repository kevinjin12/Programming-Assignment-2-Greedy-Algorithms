...
Assume, for contradiction, there exists a offline algorithm $A$ and fixed sequence $r = r_1, r_2, ..., r_m$ such that the number of misses of $A$ is strictly less than the number of misses of OPTFF on this particular sequence. Note, there might be multiple of these offline algorithms that satisfy this criterion. So without loss of generality, choose A to be the algorithm that agrees with OPTFF for the longest. More specifically, let $S_A$ be the reduced schedule produced by $A$ and $S_O$ be the reduced schedule produced by OPTFF(*1). Then $S_A$ and $S_O$ agree for the first $j$ items in the sequence of requests, where $j$ is the longest out of all such algorithms. Then consider the $(j+1)$ request to item $g = r_{j+1}$. Since they differ at this step, suppose $S_A$ evicts item $e$ while $S_O$ evicts item $f$ where $e \neq f$. By definition of the OPTFF algorithm, request $f$ must appear further in the sequence than request $e$. Now, let $S_{A'}$ be a modified schedule of $S_A$ where $S_{A'}$ evicts item $f$, like OPTFF, instead of item $e$ at step $j+1$. 

(*2)At any step $k > j + 1$, if $r_k \neq e, f$, then $r_k$ must either be in the cache for both $S_{A'}$ and $S_A$ or not in the cache for both $S_{A'}$ and $S_A$ due to the construction of $A'$ (only differ on e and f) - in either case $S_{A'}$ has no more misses than $S_A$. Additionally, since request $e$ comes before request $f$, at request $e$ there are two cases. If $S_A$ evicts $f$, then since $S_{A'}$ already has $e$ in its cache, the caches are the same and $S_{A'}$ has no more misses than $S_A$. In the case where $S_A$ evicts a different item $x$, let $S_{A'}$ also remove $x$. Then by a previous lemma proved in class, we can reduce $S_{A'}$ without extra evictions. In both cases, $S_{A'}$ has no more misses than $S_A$ and agrees with $S_O$ up until the $j+1$ step. But this is a contradiction, as $A'$ agrees with OPTFF for a longer sequence of requests than $A$ and misses of A' <= misses of A < misses of OPTFF. Hence, there cannot exist an algorithm $A$ with stricly less misses than OPTFF. Thus, for any offline algorithm that knows the full request sequence $A$ and any fixed sequence, the number of misses of OPTFF is no larger than $A$.

*1. We can do this since, we showed in class that any non-reduced schedule can be converted to a reduced schedule without any extra evictions.

*2. Also note, that instead of outlining the specifics, we could also say that because $S_A$ and $S_O$ agree up through step $j$, then by the invariant we proved in class, there is a reduced schedule $S_{A'}$ that makes the same evicts as $S_O$ through the first $j+1$ steps and has no more misses than $S_A$. But once again, we assumed $A$ to agree with OPTFF for the longest sequence of requests. So, this is a contradiction, as $A'$ agrees with OPTFF for a longer sequence of requests than $A$ and misses of A' <= misses of A < misses of OPTFF. Hence, there cannot exist an algorithm $A$ with stricly less misses than OPTFF.
